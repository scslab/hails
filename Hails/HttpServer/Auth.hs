...
personaAuth key audience app0 req0 respond = do
   case () of
    _ | doLogin -> do
        assertion <- requestBody req0
        ...
        case muser of
          ...
          Just usr -> let hmac   = T.pack $ showDigest $ hmacSha1 key
                      ...
                      in respond $ responseLBS status200
                            ...
                            ""
    _ | doLogout -> respond $ responseLBS status200
      ...
      let mauth = do cookies <- parseCookies `liftM`
          ...
          req = case mauth of
                  Just (usr, True) -> req0 { requestHeaders =
                                               ...
                                               :(requestHeaders req0) }
                  _ -> req0
      in requireLoginMiddleware (return $ respRedir req) app0 req respond
...
openIdAuth :: T.Text -- ^ OpenID Provider 
           -> Middleware
openIdAuth openIdUrl app0 req0 respond = do
  case pathInfo req0 of
    "_hails":"logout":_ -> do
      ...
      respond $ responseLBS status302 [ ("Set-Cookie", cookie)
                                     , ("Location", redirectTo)] ""
    "_hails":"login":_ -> do
      ...
      respond $ responseLBS status200 ([ ("Set-Cookie", cookie)
                                      , ("Location", redirectTo)])
                                      (L8.pack $ show qry)
    _ -> do
      ...
      let redirectResp = do
          ...
          return $ responseLBS status302 [ ("Location", (S8.pack . T.unpack $ url))
                                         , ("Set-Cookie", cookie)] ""
      requireLoginMiddleware redirectResp app0 req respond
...
requireLoginMiddleware :: IO Response -> Application -> Application
requireLoginMiddleware loginResp app0 req respond = do
  app0 req $ \appResp ->
    if hasLogin appResp && notLoggedIn
      then (loginResp >>= respond)
      else respond appResp
